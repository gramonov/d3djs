...
function loadData() {
    ...
    max_data_value = Math.max.apply(Math, [].concat.apply([], data.values));
...
function render() {
    ...
    for (var i = 0; i < plot.children.length; i++) {
        ...
        if (chance < 0.05) {
            ...
            plot.children[i].scale.z = dz;
            plot.children[i].position.z -= (plot.children[i].position.z - plot.children[i].geometry.depth * dz / 2);
            plot2.children[i].scale.z = dz;
            plot2.children[i].position.z -= (plot2.children[i].position.z - plot2.children[i].geometry.depth * dz / 2);
        }
    }
    ...
    if ( intersects.length > 0 ) {
        ...
        if ( INTERSECTED != intersects[0].object ) {
            ...
            var message = "Series: " + data.series[INTERSECTED.datum.series] + " " +
                ...
                "Value : " + INTERSECTED.geometry.depth * INTERSECTED.scale.z;
            console.log(message);
